...
const DistanceChart = ({ distances: landmarkDistances }) => {
  ...
  useEffect(() => {
    if (landmarkDistances && showChart) {
        setThumbDistance(prev => [...prev, landmarkDistances[0] < 0.1 ? landmarkDistances[0] : 0.1])
        setIndexDistance(prev => [...prev, landmarkDistances[1] < 0.1 ? landmarkDistances[1] : 0.1])
        setMiddleDistance(prev => [...prev, landmarkDistances[2] < 0.1 ? landmarkDistances[2] : 0.1])
        setRingDistance(prev => [...prev, landmarkDistances[3] < 0.1 ? landmarkDistances[3] : 0.1])
        setPinkyDistance(prev => [...prev, landmarkDistances[4] < 0.1 ? landmarkDistances[4] : 0.1])
        setWristDistance(prev => [...prev, landmarkDistances[5] < 0.1 ? landmarkDistances[5] : 0.1])
    }
  }, [landmarkDistances]);
